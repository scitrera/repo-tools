...
def _version_source_step(version_from: Optional[str], indent: str = "      ") -> str:
    """Read project version via `sync-versions --print-version`. Empty if no version_from."""
    if version_from is None:
        return ""
    return f"""{indent}- uses: {SETUP_PYTHON}
{indent}  with:
{indent}    python-version: '3.12'

{indent}- name: Read image version from versions.yaml
{indent}  id: imgver
{indent}  run: |
{indent}    pip install scitrera-repo-tools
{indent}    VER=$(sync-versions --print-version {version_from})
...
def _qemu_build_job(
    ...
    version_step = _version_source_step(img.version_from)
...
def _inline_test_jobs(config: SyncConfig, ci: CiConfig) -> tuple:
    ...
    if "python" in test_prereqs:
        for p in _python_projects(config):
            parts.append(_python_test_job(p, _project_dir(config, "python", p), ci))
            ids.append(f"test-{p}")
        ...
        if node.strategy == "qemu":
            image_jobs.append(
                _qemu_build_job(
                    ...
                    docker_cfg, ci_docker, platforms_csv,
                )
            )
        else:  # native
            ...
            image_jobs.append(
                _native_merge_job(node, docker_cfg, ci_docker, platforms)
            )
