#!/usr/bin/env python3
"""Drop-in shim for scitrera-repo-tools `directory-split`.

Copy this file into any repo's `scripts/` directory (or run it from anywhere)
to split a directory into N approximately-equal buckets via greedy bin-packing.
...
Usage:
    python scripts/directory-split.py <directory> <num_parts>
    python scripts/directory-split.py ./data 4 --exclude "*.log"

All flags pass through to `directory-split`.
"""
...
def _try_uvx(args: List[str]) -> None:
    ...
    if name == "uv":
        cmd = [uv, "tool", "run", "--from", source, "directory-split", *args]
    else:
        cmd = [uv, "--from", source, "directory-split", *args]
    os.execvp(cmd[0], cmd)  # never returns


def _try_import(args: List[str]) -> bool:
    try:
        from scitrera_repo_tools.directory_split import main as split_main
    except ImportError:
        return False
    sys.exit(split_main(args))
